...
```{r}
# Standalone function to filter TF → gene interactions
filter_TF_gene_edges <- function(network, tf_activity, gene_expression, tf_regulon) {
  # network: data.frame with columns source, interaction (+1/-1), target
  # tf_activity: named vector, TF names as names, activity as values
  # gene_expression: named vector, gene names as names, binarized expression (-1,0,1)
  # tf_regulon: data.frame with columns tf, target, sign (+1/-1)
  
  # Keep only TFs in the regulon
  tf_activity <- tf_activity[names(tf_activity) %in% tf_regulon$tf]
  
  # Build a data frame for genes
  gene_exp_df <- data.frame(
    target = names(gene_expression),
    target_sign = gene_expression,
    stringsAsFactors = FALSE
  )
  
  # Build a data frame for TFs
  tf_df <- data.frame(
    source = names(tf_activity),
    TF_sign = tf_activity,
    stringsAsFactors = FALSE
  )
  
  # Annotate network with gene expression and TF activity
  annotated_network <- merge(network, gene_exp_df, by = "target", all.x = TRUE)
  annotated_network <- merge(annotated_network, tf_df, by = "source", all.x = TRUE)
  
  # Flag edges
  annotated_network$source_is_TF <- annotated_network$source %in% tf_regulon$tf
  annotated_network$target_gene_unchanged <- annotated_network$target_sign == 0
  annotated_network$TF_target_unchanged <- annotated_network$source_is_TF & 
    (annotated_network$target_gene_unchanged | is.na(annotated_network$target_gene_unchanged))
  annotated_network$inconsistent_TF_gene_sign <- annotated_network$source_is_TF & 
    !is.na(annotated_network$TF_sign) & !is.na(annotated_network$target_sign) &
    (sign(annotated_network$TF_sign) != annotated_network$interaction * sign(annotated_network$target_sign))
  
  # Remove inconsistent or unchanged edges
  removed_edges <- annotated_network[annotated_network$TF_target_unchanged | annotated_network$inconsistent_TF_gene_sign, ]
  message("Removed ", nrow(removed_edges), " inconsistent TF → gene edges.")
  
  # Keep consistent edges
  kept_edges <- annotated_network[!(annotated_network$TF_target_unchanged | annotated_network$inconsistent_TF_gene_sign), ]
  return(kept_edges[, c("source", "interaction", "target")])
}
```
...
# Example PKN
network <- data.frame(
  source = c("TF1","TF2","TF3"),
  interaction = c(1,-1,1),
  target = c("GeneA","GeneB","GeneC")
)

# TF activities
tf_activity <- c(TF1 = 1, TF2 = 1, TF3 = 0)

# Gene expression (binarized)
gene_expression <- c(GeneA = 1, GeneB = 1, GeneC = 0)
...
# Run filter
filtered_network <- filter_TF_gene_edges(network, tf_activity, gene_expression, tf_regulon)
